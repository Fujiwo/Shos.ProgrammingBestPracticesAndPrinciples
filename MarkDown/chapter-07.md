...
>「動くコードに手を加えるのは危険だ」<br>
>「時間がないから、リファクタリングなんてやってられない」

──これらは開発現場でよく聞かれる言葉である。

しかし、リファクタリングを行わないコードは必然的に腐敗し、やがて保守不可能な状態に陥る。この章で学ぶ継続的品質改善は、あなたとあなたのチームを「レガシーコードの泥沼」から救い出す実践的な救命策である。美しいコードは一日にして成らず──しかし、適切なリファクタリング戦略があれば必ず実現できる。

リファクタリングは、コードを美しく保つために必要な作業である。

コードを美しく保つ、つまり、**変更のしやすさ**と**テストのしやすさ**を維持するということである。大切なことなので何度もいうが、開発は最初の1行を書き始めてから、**変更**と**テスト**を繰り返すことになる。つまり、開発が継続できる(sustainable)ためには、リファクタリングが肝となる。

### 例.

簡易的なCADを作成する例をあげよう。

まず線分を追加するためのControllerクラスを作成する:
...
public class Controller
{
    ...
    public void MouseUp(Point location)
        => Model.Add(new LineFigure(Start: firstPoint, End: location));
}
```
...
public class Controller
{
    public CommandKind CommandKind { get; set; }  = CommandKind.Line;
    ...
    public void MouseUp(Point location)
    {
        switch (CommandKind)
        {
            case CommandKind.Line     :
                ...
                break;
            case CommandKind.Rectangle:
                Model.Add(new RectangleFigure(Rectangle.FromLTRB(firstPoint.X, firstPoint.Y, location.X, location.Y)));
                break;
            case CommandKind.Select:
                Model.SelectFigure(location);
                break;
            default:
                break;
        }
...
リファクタリングして、今後のコマンドの追加に耐えうる設計に変えてみよう。

ここでは、Strategyパターンを用いてみる:

_[C#]_
```csharp
public abstract class Command
{
    public virtual void OnDragStart(Model model, Point location) {}
    public virtual void OnDragEnd  (Model model, Point location) {}
}
...
public class AddLineCommand : Command
{
    ...
    public override void OnDragStart(Model model, Point location) => firstPoint = location;
    public override void OnDragEnd  (Model model, Point location)
        => model.Add(new LineFigure(Start: firstPoint, End: location));
}

public class AddRectangleCommand : Command
{
    Point firstPoint;
...
{
    public override void OnDragEnd(Model model, Point location) => model.SelectFigure(location);
}

public class Controller
{
    public Command Command { get; set; } = new AddLineCommand();

    public required Model Model { get; init; }

    public void MouseDown(Point location) => Command.OnDragStart(Model, location);
    public void MouseUp  (Point location) => Command.OnDragEnd  (Model, location);
}
```
...
public abstract class Add2PointFigureCommand : Command
{
    Point firstPoint;

    public override void OnDragStart(Model model, Point location) => firstPoint = location;
    public override void OnDragEnd  (Model model, Point location) => model.Add(CreateFigure(firstPoint, location));

    protected abstract Figure CreateFigure(Point point1, Point point2);
}

public class AddLineCommand : Add2PointFigureCommand
{
    protected override Figure CreateFigure(Point point1, Point point2)
        => new LineFigure(Start: point1, End: point2);
}

public class AddRectangleCommand : Add2PointFigureCommand
{
    protected override Figure CreateFigure(Point point1, Point point2)
        => new RectangleFigure(Rectangle.FromLTRB(point1.X, point1.Y, point2.X, point2.Y));
}
...
classDiagram
    ...
    Controller --> Command
    Command <|-- Add2PointFigureCommand
    Add2PointFigureCommand <|-- AddLineCommand
    Add2PointFigureCommand <|-- AddRectangleCommand
    Command <|-- SelectCommand
```

リファクタリングなので、まだ機能は増えていない。リファクタリング前と外から見た動きは同じである。

だが、このリファクタリングをしておくと、次に楕円の追加コマンドを機能追加するときに、Controller部に追記するコードは下記の部分だけとなる:

_[C#]_
```csharp

public class AddEllipseCommand : Add2PointFigureCommand
{
    protected override Figure CreateFigure(Point point1, Point point2)
        => new EllipseFigure(Rectangle.FromLTRB(point1.X, point1.Y, point2.X, point2.Y));
}
```
